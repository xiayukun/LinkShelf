...
## 2.0.1 维护发布完成（2026-09-16）

- 以远端 main 为基线，在隔离工作区审查并合并 Dependabot PR #4/#5；原工作区已有文档改动保持不动。
- 构建/发布工作流更新 checkout v7、setup-dotnet v6，不调整 .NET 8 或迁移/链接行为。版本从 2.0.0 升至 2.0.1；CLI 原先写死的 1.1.6 改为读取程序集版本。
- Release 构建和单文件发布通过，v2.0.1 标签指向发布提交 533add4，main/tag 构建及 release 工作流全部通过。GitHub Release 仅含 LinkShelf.exe，已下载核对 SHA-256 `07e96d0413d70687a84446a7a3a38e18cf84bb595977dcd91fc7b0643ef84ac8`、CLI 2.0.1、文件版本 2.0.1.0 和隔离 `check --json`。
- 用户明确授权更新本机三份旧程序。三份均已替换为从同一发布提交本地构建的 2.0.1：两份投射程序与主工作区 dist/LinkShelf.exe 保持硬链接，另一份保持独立副本；两份现有配置的哈希未变，旧 EXE 留有临时回退副本。本地产物 SHA-256 为 `f4bcf1558e2027c14feb8a9c8a99e36c485a32dfdeed4be1689ee009a134d3b4`，与 GitHub runner 独立构建的资产不同，未声称两种构建逐字节一致。
- 以下早期交接中的 v1.1.x 版本和本机路径属于历史记录，不能作为当前发布状态。
