...
English: [user-guide.en.md](user-guide.en.md) | 返回主页：[README.md](../README.md)
...
如果只是想让脚本或 AI 助手查看本机当前可添加的推荐路径，可以运行：

```powershell
.\LinkShelf.exe recommended --json
```

这个命令同样只读，不会移动、删除、恢复或创建链接。
...
如果我要你推荐可收纳路径，先运行 `LinkShelf.exe recommended --json` 查看本机实际存在且尚未管理的推荐路径。
...
- 需要推荐可收纳路径时，读取 `recommended --json` 输出；它只列出本机存在、尚未被启用配置覆盖的推荐项。
