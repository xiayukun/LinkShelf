...
- 当 Windows 因为文件占用返回拒绝访问时，检测正在使用该路径的进程，并可结束进程后重试。
...
推荐项目只会显示本机存在、并且尚未写入 `link-shelf.config.json` 的路径。选择推荐项目后，会执行和手动选择该路径一样的添加流程。
...
选中一行或多行后点击 `搬回原位/撤销`，Link Shelf 会移除原路径链接，把缓存内容搬回原始位置，并删除配置记录。只有当原始位置不存在，或仍然是指向该缓存项的正确链接时才会执行。如果原始位置已经有真实内容，程序会跳过撤销，避免覆盖用户数据。
